...
import "./INikaTokenV2.sol";

/*
 * NIKA is Angstrom's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract NikaTokenV2 is Ownable, ERC20("Nika token", "NIKA"), INikaTokenV2 {
    ...
    modifier onlyMaster() {
        require(
            ...
            "NikaToken: caller is not the master"
        );
        _;
    }
    ...
    function masterEmissionRate() public view override returns (uint256) {
        return
            emissionRate.mul(farmingAllocation.add(legacyAllocation)).div(
    ...
    /**
     * @dev Burns "amount" of NIKA by sending it to BURN_ADDRESS
     */
    ...
    /**
     * @dev Updates NIKA emission rate per second
     *
     * Must only be called by the owner
     */
