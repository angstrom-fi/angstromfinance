...
import "./IAngstromRouter.sol";
import "./UniswapV2Library.sol";
import "./SafeMath.sol";
import "./IWETH.sol";

contract AngstromRouter is IAngstromRouter {
    using SafeMath for uint;
    address public immutable override factory;
    address public immutable override WETH;

    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, "AngstromRouter: EXPIRED");
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (IAngstromFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IAngstromFactory(factory).createPair(tokenA, tokenB);
        }
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "AngstromRouter: INSUFFICIENT_B_AMOUNT"
                );
                ...
                require(
                    ...
                    "AngstromRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint amountA, uint amountB, uint liquidity)
    {
        ...
        liquidity = IAngstromPair(pair).mint(to);
    }
    ...
    function removeLiquidity(
        ...
        address pair = UniswapV2Library.pairFor(factory, tokenA, tokenB);
        IAngstromPair(pair).transferFrom(msg.sender, pair, liquidity);
        // send liquidity to pair
        (uint amount0, uint amount1) = IAngstromPair(pair).burn(to);
        (address token0, ) = UniswapV2Library.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0
            ? (amount0, amount1)
            : (amount1, amount0);
        require(amountA >= amountAMin, "AngstromRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "AngstromRouter: INSUFFICIENT_B_AMOUNT");
    }
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            IAngstromPair pair = IAngstromPair(
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >=
                amountOutMin,
            "AngstromRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(path[0] == WETH, "AngstromRouter: INVALID_PATH");
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(path[path.length - 1] == WETH, "AngstromRouter: INVALID_PATH");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            UniswapV2Library.pairFor(factory, path[0], path[1]),
