...
contract AngstromPair is IAngstromPair, UniswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, "AngstromPair: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function setFeePercent(
        ...
        require(
            msg.sender == IAngstromFactory(factory).feePercentOwner(),
            "AngstromPair: only factory's feeAmountOwner"
        );
        require(
            newToken0FeePercent <= MAX_FEE_PERCENT &&
                newToken1FeePercent <= MAX_FEE_PERCENT,
            "AngstromPair: feePercent mustn't exceed the maximum"
        );
        require(
            newToken0FeePercent > 0 && newToken1FeePercent > 0,
            "AngstromPair: feePercent mustn't exceed the minimum"
        );
    ...
    function setStableSwap(
        ...
        require(
            msg.sender == IAngstromFactory(factory).setStableOwner(),
            "AngstromPair: only factory's setStableOwner"
        );
        require(!pairTypeImmutable, "AngstromPair: immutable");

        require(stable != stableSwap, "AngstromPair: no update");
        require(
    ...
    function setPairTypeImmutable() external lock {
        require(
            msg.sender == IAngstromFactory(factory).owner(),
            "AngstromPair: only factory's owner"
        );
        require(!pairTypeImmutable, "AngstromPair: already immutable");
    ...
    function _mintFee(
        ...
        (uint ownerFeeShare, address feeTo) = IAngstromFactory(factory)
            .feeInfo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, "AngstromPair: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "AngstromPair: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function _swap(
        ...
        require(
            ...
            "AngstromPair: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        ...
        require(
            ...
            "AngstromPair: INSUFFICIENT_LIQUIDITY"
        );

        {
            require(
                to != tokensData.token0 && to != tokensData.token1,
                "AngstromPair: INVALID_TO"
            );
        ...
        require(
            ...
            "AngstromPair: INSUFFICIENT_INPUT_AMOUNT"
        );
...

        {
            ...
            uint referrerInputFeeShare = referrer != address(0)
                ? IAngstromFactory(factory).referrersFeeShare(referrer)
                : 0;
    ...
    function sync() external lock {
        ...
        require(
            ...
            "AngstromPair: liquidity ratio not initialized"
        );
